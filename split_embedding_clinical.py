...
my_df = pd.read_csv('./embeddings_clinical.csv')
embeddings = []
...
folder = os.path.exists("./data_clinical")
if not folder:
    os.makedirs("./data_clinical")

my_df.to_csv("my_clinical.csv", index=False)
...
for ii in range(bestKValue):
    ...
    a.to_csv('./data_clinical/' + 'data' + str(ii) + '.csv')
...
mean_.to_csv('./data_clinical/' + 'mean' + '.csv')
