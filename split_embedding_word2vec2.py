...
my_df = pd.read_csv('./embeddings_word2vec_finetuned.csv')
embeddings = []
...
folder = os.path.exists("./data_word2vec_finetuned")
if not folder:
    os.makedirs("./data_word2vec_finetuned")

my_df.to_csv("my_word2vec_finetuned.csv", index=False)
...
for ii in range(bestKValue):
    ...
    a.to_csv('./data_word2vec_finetuned/' + 'data' + str(ii) + '.csv')
    tmp = my_df[my_df.label == ii].embeddings.tolist()
    for t in tmp:
        t0 = list(map(float, t[1:-1].split(', ')))
        b.append(t0)
    b = np.mean(b, axis=0)
    label.append(ii)
...
mean_.to_csv('./data_word2vec_finetuned/' + 'mean' + '.csv')
