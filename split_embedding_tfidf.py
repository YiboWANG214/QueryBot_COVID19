...
my_df = pd.read_csv('./embeddings_tfidf.csv')
embeddings = []
for j in range(len(my_df)):
    ...
    sentence = np.array(sentence).reshape(-1, 500)
...
folder = os.path.exists("./data_tfidf")
if not folder:
    os.makedirs("./data_tfidf")

my_df.to_csv("my_tfidf.csv", index=False)
...
for ii in range(bestKValue):
    ...
    a.to_csv('./data_tfidf/' + 'data' + str(ii) + '.csv')
...
mean_.to_csv('./data_tfidf/' + 'mean' + '.csv')
