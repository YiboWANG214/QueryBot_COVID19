...
model = Word2Vec.load('finetuned_word2vec.model')
...
def embedding(sent):
    line = nltk.word_tokenize(clean(sent))
    tmp = [[0]*200]
    for word in line:
        if word in model.wv.vocab:
            tmp.append(model[word])
    sentence_embedding = np.mean(tmp, axis=0)
...
def calculate(original_q, data_):
    q = clean(original_q)
    files = os.listdir('./records_word2vec/json/relevant')
    sentence_embedding_relevant = np.zeros(200)
    sentence_embedding_non = np.zeros(200)
    for json_file in files:
        if q.replace(' ', '') + '.json' == json_file:
            if not os.path.exists('./records_word2vec/json/relevant/' + json_file):
                os.makedirs('./records_word2vec/json/relevant/' + json_file)
                with open('./records_word2vec/json/relevant/' + json_file, 'a') as ffile:
                    ffile.write(original_q)
            with open('./records_word2vec/json/relevant/' + json_file) as load_f:
                ff = load_f.readlines()
            ...
            break
    files2 = os.listdir('./records_word2vec/json/non')
    for json_file in files2:
        if q.replace(' ', '')+'.json' == json_file:
            if not os.path.exists('./records_word2vec/json/non/'+json_file):
                os.makedirs('./records_word2vec/json/non/'+json_file)
                with open('./records_word2vec/json/non/'+json_file, 'a') as ffile2:
                    ffile2.write(original_q)
            with open('./records_word2vec/json/non/'+json_file) as load_f2:
    ...
    query_en = np.array(query_en).reshape(-1, 200)
    ...
    for j in range(len(data_)):
        ...
        sentence = np.array(sentence).reshape(-1, 200)
...
def calculate2(original_q, data_):
    ...
    for json_file in files2:
        if q.replace(' ', '')+'.json' == json_file:
            if not os.path.exists('./records_word2vec_/json/non/'+json_file):
                if not os.path.exists('./records_word2vec/json/non/'+json_file):
                    os.makedirs('./records_word2vec/json/non/'+json_file)
                    with open('./records_word2vec/json/non/'+json_file, 'a') as ffile2:
                        ffile2.write(original_q)
...
def func():
    ...
    if str2 != '':
        data = pd.read_csv("./embeddings_word2vec_finetuned.csv")
    ...
    # without patient information
        ...
        data_mean = pd.read_csv('./data_word2vec_finetuned/mean.csv')
        label = calculate2(str1, data_mean)
        print(label)
        file_name = './data_word2vec_finetuned/data'+str(label)+'.csv'
...
def func2(sentences_, s):
    ...
    def func3():
        ...
        with open('./records_word2vec/json/relevant/' + str1.replace(' ', '') + '.json', "a", encoding='utf-8') as f:
        ...
        with open('./records_word2vec/json/non/' + str1.replace(' ', '') + '.json', "a", encoding='utf-8') as f:
...
folder = os.path.exists('./records_word2vec/json/relevant')
if not folder:
    os.makedirs('./records_word2vec/json/relevant')
if not os.path.exists('./records_word2vec/json/non'):
    os.makedirs('./records_word2vec/json/non')
folder2 = os.path.exists('./records_word2vec/csv')
if not folder2:
    os.makedirs('./records_word2vec/csv')
