...
def calculate(original_q, data_):
    q = clean(original_q)
    files = os.listdir('./records_bio/json/relevant')
    sentence_embedding_relevant = np.zeros(768)
    sentence_embedding_non = np.zeros(768)
    for json_file in files:
        if q.replace(' ', '') + '.json' == json_file:
            if not os.path.exists('./records_bio/json/relevant/' + json_file):
                os.makedirs('./records_bio/json/relevant/' + json_file)
                with open('./records_bio/json/relevant/' + json_file, 'a') as ffile:
                    ffile.write(original_q)
            with open('./records_bio/json/relevant/' + json_file) as load_f:
                ff = load_f.readlines()
            ...
            break
    files2 = os.listdir('./records_bio/json/non')
    for json_file in files2:
        if q.replace(' ', '')+'.json' == json_file:
            if not os.path.exists('./records_bio/json/non/'+json_file):
                os.makedirs('./records_bio/json/non/'+json_file)
                with open('./records_bio/json/non/'+json_file, 'a') as ffile2:
                    ffile2.write(original_q)
            with open('./records_bio/json/non/'+json_file) as load_f2:
    ...
    sent = []
    with open('./records_bio/csv/'+q+'.csv', "a") as csvfile:
...
def func():
    ...
    if str2 != '':
        data = pd.read_csv("./embeddings_bio.csv")
    ...
    # without patient information
        ...
        data_mean = pd.read_csv('./data_bio/mean.csv')
        label = calculate2(str1, data_mean)
        print(label)
        file_name = './data_bio/data'+str(label)+'.csv'
...
def func2(sentences_, s):
    ...
    def func3():
        ...
        with open('./records_bio/json/relevant/' + str1.replace(' ', '') + '.json', "a", encoding='utf-8') as f:
        ...
        with open('./records_bio/json/non/' + str1.replace(' ', '') + '.json', "a", encoding='utf-8') as f:
...
# data = pd.read_csv('./embeddings500_768_noteID_3.csv')
# print(data.columns)
file = open('file.txt', 'a')
folder = os.path.exists('./records_bio/json/relevant')
if not folder:
    os.makedirs('./records_bio/json/relevant')
if not os.path.exists('./records_bio/json/non'):
    os.makedirs('./records_bio/json/non')
folder2 = os.path.exists('./records_bio/csv')
if not folder2:
    os.makedirs('./records_bio/csv')
